...
from utils.sg2im.utils import Resize, imagenet_preprocess
...
COCO_C2I = {c: i for i, c in enumerate(COCO_CLASSES)}

COCO_CLASSES_PAD = ['NONE', 'person', 'bicycle', 'car', 'motorcycle', 'airplane', 'bus', 'train', 'truck', 'boat',
                    ...
                    'NONE', 'handbag', 'tie', 'suitcase', 'frisbee', 'skis', 'snowboard', 'sports ball', 'kite', 'baseball bat',
                    'baseball glove', 'skateboard', 'surfboard', 'tennis racket',
                    'bottle', 'NONE', 'wine glass', 'cup', 'fork', 'knife', 'spoon', 'bowl', 'banana', 'apple', 'sandwich',
                    'orange', 'broccoli', 'carrot', 'hot dog', 'pizza', 'donut', 'cake', 'chair', 'couch', 'potted plant',
                    'bed', 'NONE', 'dining table', 'NONE', 'NONE', 'toilet', 'NONE', 'tv', 'laptop', 'mouse', 'remote',
                    'keyboard', 'cell phone', 'microwave', 'oven', 'toaster', 'sink', 'refrigerator', 'NONE', 'book', 'clock',
                    'vase', 'scissors', 'teddy bear', 'hair drier', 'toothbrush']


class COCODataModule(BaseDataModule):
    def __init__(self, data_dir="data", **kwargs):
        super(COCODataModule, self).__init__(data_dir)

        graph_data = kwargs.pop('graph_data', True)
        self.pos_enc_dim = kwargs.pop('pos_enc_dim', 8)

        train_image_dir = 'data/coco/train2017'
        train_inst_json = 'data/coco/annotations/instances_train2017.json'
        train_stuff_json = 'data/coco/annotations/stuff_train2017.json'

        val_image_dir = 'data/coco/val2017'
        val_inst_json = 'data/coco/annotations/instances_val2017.json'
        val_stuff_json = 'data/coco/annotations/stuff_val2017.json'
        self.train_ds = CocoSceneGraphDataset(
            train_image_dir, train_inst_json, train_stuff_json, stuff_only=False, **kwargs)
        self.val_ds = CocoSceneGraphDataset(
            val_image_dir, val_inst_json, val_stuff_json, stuff_only=False, **kwargs)
        ...
        if graph_data:
            ...
            self.collate_fn = partial(
                coco_collate_fn, vocab=self.train_ds.vocab)
...
def build_coco_dsets(args):
    ...
    dset_kwargs = {
        'image_dir': data_dir / args.coco_train_image_dir,
        'instances_json': data_dir / args.coco_train_instances_json,
    ...
    dset_kwargs['image_dir'] = data_dir / args.coco_val_image_dir
    dset_kwargs['instances_json'] = data_dir / args.coco_val_instances_json
...
def coco_original_collate_fn(batch):
    """
    Collate function to be used when wrapping CocoSceneGraphDataset in a
    DataLoader. Returns a tuple of the following:
...
def coco_collate_fn(batch, vocab=None):
    ...
    for i, (img, objs, boxes, masks, triplets) in enumerate(batch):
        O = objs.size(0)
        T = triplets.size(0)
    ...
    for i, (img, objs, boxes, masks, triplets) in enumerate(batch):
        all_imgs.append(img[None])
        O, T = objs.size(0), triplets.size(0)

        # Padded objects
        if max_objects - O > 0:
            zeros_v = torch.zeros(max_objects - O, dtype=torch.long)
            objs = torch.cat([objs, zeros_v])

        # Padded boxes
        if max_objects - O > 0:
            padded_boxes = torch.FloatTensor(
                [[-1, -1, -1, -1]]).repeat(max_objects - O, 1)
            boxes = torch.cat([boxes, padded_boxes])

        # Padded masks
        if masks is not None and max_objects - O > 0:
            padded_masks = torch.zeros(
                [max_objects - O, masks.size(1), masks.size(2)]).type(torch.LongTensor)
            masks = torch.cat([masks, padded_masks])

        # Padded triplets
        if max_triplets - T > 0:
            padded_triplets = torch.LongTensor([[0, 0, 0]]).repeat(
                max_triplets - T, 1)
